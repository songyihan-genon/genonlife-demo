...
import { AUDIT_CASES, TOTAL_QA_PAIRS } from "@/lib/audit/mock-data"
import type { AuditCase, AuditStatus } from "@/lib/audit/types"
...
export function CaseList({ selectedId, onSelect }: CaseListProps) {
  return (
    <div className="flex flex-col rounded-lg border border-border bg-card shadow-sm">
      <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
        <h2 className="text-sm font-semibold text-foreground">오안내 대상 리스트</h2>
        <p className="text-xs text-muted-foreground">
          총 <span className="font-semibold text-foreground">{AUDIT_CASES.length}건</span>{" "}
          (검수 대상 {TOTAL_QA_PAIRS.toLocaleString()}건 중)
        </p>
      </div>
        ...
        <table className="w-full caption-bottom text-sm">
          <thead className="sticky top-0 z-10 bg-input text-xs uppercase tracking-wider text-muted-foreground">
            <tr className="border-b border-border">
              ...
              <Th className="w-[120px]">상담사</Th>
              <Th className="w-[100px]">접촉시점</Th>
              <Th>오안내 평가 사유</Th>
