...
export async function GET(request: NextRequest, context: { params: { jobId: string } }) {
  ...
  try {
    const backendUrl = new URL(`${BASE_URL}/jobs/${jobId}/result`)
    ...
    console.error("[translation] failed to download result:", error)
    return NextResponse.json({ detail: "Failed to download translated document" }, { status: 500 })
  }
}
